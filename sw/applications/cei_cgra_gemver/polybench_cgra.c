...
{
    // A = A + u1 * v1 + u2 * v2
    gemver_0(GEMVER_N, A, u1, v1, u2, v2);

    // x = beta * A_t * y
    mult_cte_m_v_kernel[ 8] = *beta;
    mult_cte_m_v_kernel[13] = *beta;
    mult_cte_m_v_kernel[18] = *beta;
    mult_cte_m_v_kernel[44] = (mult_cte_m_v_kernel[44] & 0xFFFF0000) | GEMVER_N;
    mult_cte_m_v_kernel[49] = (mult_cte_m_v_kernel[49] & 0xFFFF0000) | GEMVER_N;
    mult_cte_m_v_kernel[54] = (mult_cte_m_v_kernel[54] & 0xFFFF0000) | GEMVER_N;
    gemver_1(GEMVER_N, A, y, x);

    // x = x + z
    gemver_2(GEMVER_N, x, z);

    // w = alpha * A * x
    mult_cte_m_v_kernel[ 8] = *alpha;
    mult_cte_m_v_kernel[13] = *alpha;
    mult_cte_m_v_kernel[18] = *alpha;
    gemver_3(GEMVER_N, A, x, w);
}
