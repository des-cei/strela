    ...
    DATA_TYPE *D)
{
    ...
    mult_cte_m_v_kernel[44] = (mult_cte_m_v_kernel[44] & 0xFFFF0000) | TWOMM_NK;
    mult_cte_m_v_kernel[49] = (mult_cte_m_v_kernel[49] & 0xFFFF0000) | TWOMM_NK;
    mult_cte_m_v_kernel[54] = (mult_cte_m_v_kernel[54] & 0xFFFF0000) | TWOMM_NK;

    mult_cte_m_v_cgra(TWOMM_NI, TWOMM_NJ, TWOMM_NK, A, B, tmp);

    // M = (alpha*A*B_t) * C
    dot_product_kernel[24] = (dot_product_kernel[24] & 0xFFFF0000) | TWOMM_NJ;
    dot_product_kernel[29] = (dot_product_kernel[29] & 0xFFFF0000) | TWOMM_NJ;
    dot_product_kernel[34] = (dot_product_kernel[34] & 0xFFFF0000) | TWOMM_NJ;

    matmul(TWOMM_NI, TWOMM_NL, TWOMM_NJ, tmp, C, M);

    // D = (alpha*A*B*C) + beta*D
    sum_cte_v_v_kernel[ 8] = *beta;
    sum_cte_v_v_kernel[18] = *beta;

    sum_cte_v_v_cgra(TWOMM_NI, TWOMM_NL, M, D);
}
